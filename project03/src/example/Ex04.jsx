...
const Ex04 = () => {
     ...
     */
        ...
        // myDice나 comDice로 변화를 감지할 경우, 적은 확률이지만 두개가 두판다 같은 값이 나올수도 있음
        // => 그럴 때는 감지X
        const makeRandom = () => {
        ...
        return (
            <div className='box'>
                ...
                <div>
                    ...
                    <Button variant="success" onClick={resetDice}> RESET </Button>
                </div>    
    
                <div className='board-container'>
                    <Board num={myDice} name="나"/>
                    <Board num={comDice} name="컴퓨터"/>
                </div>

                {/* useEffect 배우기 이전 작성 */}
                {/*     
                <div>
                    {myDice > comDice ? '승리!' : '패배..'}
                </div> */}
                <h2>{result}</h2>
            </div>
    )
