...
const Ex08 = () => {
    ...
    /* Context란? 리액트 컴포넌트 간에 값을 전역적으로 공유할 수 있게 해주는 기능 
        ...
        A. 깊숙히 위치한 컴포넌트에 데이터를 전달해야하는 경우 
            => 여러 컴포넌트를 연달아서 props 설정 
            => 이 과정에서 실수가 잦음 
            이런 현상을 props drilling 이라고 부름 
    
            그래서 전역적으로 데이터를 보관해줄 context 를 생성해서 관리한다. 
    
        [만드는 순서]
        1. context 파일을 만들어준다. 
            - createContext (꼭 import 까지 함께)
            - export 
        ...
        4. 데이터를 쓰고싶은 곳에서 useContext를 통해 데이터를 가져온다. 
            const { 쓰고싶은데이터 } = useContext(Context경로)

    */

    
    // context provider를 적는 부분에 state관리 
